...
Прародитель - rBlog, который просто требовал полного рефакторинга с учетом всех дополнений и изменений.
Фактически, это rBlog2.

Класс полностью зависит от набора классов rLib
...
class rBlog{
	...
	* @return void
	*/
	function __construct($site, $settings = array()){
		$this->site = $site;
		$this->db = $site->db;
		$this->assignDefaultSettings();
		$this->setSettings($settings);

		$this->postsDB = new rTableClass($this->site->db, $this->settings['posts_table']);
		$this->blogsDB = new rTableClass($this->site->db, $this->settings['blogs_table']);
	...
	*/
	public function post($data){
		...
		if($cutPos !== FALSE){
			...
			$post_data['main_text'] = $this->process_text(substr($data['text'], 0, $cutPos));
			$post_data['full_text'] = $this->process_text(str_replace($this->getSetting('cutTag'), '', $data['text']));
			$post_data['have_cut'] = 1;
		}else{
			// без ката
			$post_data['main_text'] = $post_data['full_text'] = $this->process_text($data['text']);
		}
		...
		if(!empty($data['preview'])){
			...
			if($data['preview']){
				$post_data['main_text'] = $this->process_text($data['preview']);
				$post_data['have_cut'] = !strcasecmp(
					preg_replace('/\s+/', '', html_entity_decode(str_replace('&nbsp;', ' ', strip_tags($post_data['main_text'])), ENT_QUOTES, 'UTF-8')),
					preg_replace('/\s+/', '', html_entity_decode(str_replace('&nbsp;', ' ', strip_tags($post_data['full_text'])), ENT_QUOTES, 'UTF-8'))
				) ? 0 : 1;
			}
		}
		...
		return $this->postsDB->get($postID);
	}
	...
	*/
	function editPost($postID, $data){
		...
		if($cutPos !== FALSE){
			...
			$post_data['main_text'] = $this->process_text(rtrim(substr($data['text'], 0, $cutPos), ' <'));
			$post_data['full_text'] = $this->process_text(str_replace($this->getSetting('cutTag'), '', $data['text']));
			$post_data['have_cut'] = 1;
		}else{
			// без ката
        ...
        if(isset($data['tags']) && !$this->tagsObj->isTagsEq($data['tags'], $post['tags_cache'])){	
            $this->tagsObj->setEntryID($postID);
			$this->tagsObj->clear();
			$post_data['tags_cache'] = $this->tagsObj->addTags($data['tags'], $this->site->user->getID());
        }
	...
	*/
	function deletePost($postID, $checkOwner = false){
		$post = $this->getByID($postID);
		if(!$post) return false;
		if( $checkOwner && ($post['owner_id'] != $this->site->user->getID())) return false;
	...
	*/
	function votePost($postID, $vote, $uid = 0){
		if(!$uid) $uid = $this->site->user->getID();
		if(!$uid) return false;
	...
	*/
	function toggleFavorite($postID){
		if(!$this->site->user->authed()) return false;
		$uid = $this->site->user->getID();
		...
		$this->settings = array(
			...
			'selectPosts' =>
				'SELECT p.*, b.name AS blog_name, b.url AS blog_url, u.full_name AS author_name, fav.dateadd AS in_favorites
					...
					LEFT JOIN blog_favorites fav ON (fav.post_id = p.id AND fav.user_id = '.$this->site->user->getID().')',
	...
	protected function _reformSelects(){
		$this->settings['selectPosts'] =
			'SELECT p.*, b.name AS blog_name, b.url AS blog_url, u.full_name AS author_name, fav.dateadd AS in_favorites
				...
				LEFT JOIN users u ON u.id = p.owner_id
				LEFT JOIN `'.$this->settings['favorites_table'].'` fav ON (fav.post_id = p.id AND fav.user_id = '.$this->site->user->getID().')';
	...
	*/
	public function initComments(){
		...
		$this->site->addJS('comments.js');
		$this->site->addCSS('comments/comments.css');
	...
	*/
	...
	public function pos2LJ($post, $postId){
		...
		if(isset($post['postID'])){ //если обновление поста добовляем id обновляемого поста 
			$lj_post_row = $this->db->selectRow("SELECT id, ext_id FROM blog_posts_ext WHERE post_id = ?d", $postId);
		...
		if($post['todo'] == 'publish'){
			...
			if(!empty($lj_post_row)) $lj_post['itemid'] = $lj_post_row['ext_id'];
				...
				if(isset($lj_res['itemid'])){
					$this->db->query("INSERT INTO blog_posts_ext SET ?a", 
					array('type_id'=> 2, 'ext_id'=> $lj_res['itemid'], 'post_id' => $postId));
				}
