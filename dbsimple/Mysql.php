 ...
 * 
 * @version 2.x $Id: Mysql.php,v 1.1 2007/11/18 12:40:44 steel Exp $
 */
...
class DbSimple_Mysql extends DbSimple_Generic_Database
{
    ...
    function nextID($seq)
    {
        ...
        do{
            $repeat = 0;
            $this->query('LOCK TABLE ?# WRITE', $seqname);
            @$res = @$this->query("UPDATE ?# SET id=LAST_INSERT_ID(id+1)", $seqname);
            $id = mysql_insert_id();
...
//                $repeat = 1;
    ...
    function createSequence($seqname){
        $this->query('DROP TABLE IF EXISTS ' . $seqname);
        $res = $this->query('CREATE TABLE ' . $seqname
            . ' (id INTEGER UNSIGNED AUTO_INCREMENT NOT NULL,'
            . ' PRIMARY KEY(id))');        
        $this->query("INSERT INTO ${seqname} (id) VALUES (0)");
        $this->query("UPDATE ${seqname} SET id = 0");
    }
