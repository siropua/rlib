...
function quicky_block_section($params,$content,$compiler)
{
 ...
 if (!isset($params['name'])) {return $compiler->_syntax_error('Missing parameter \'name\' in section tag.');}
 if (!isset($params['loop'])) {return $compiler->_syntax_error('Missing parameter \'loop\' in section tag.');}
 else {$params['loop'] = $compiler->_dequote($params['loop']);}
 if (!isset($params['start'])) {$params['start'] = '0';}
 else {$params['start'] = $compiler->_dequote($params['start']);}
 if (!isset($params['max'])) {$params['max'] = '-1';}
 else {$params['max'] = $compiler->_dequote($params['max']);}
 if (!isset($params['step'])) {$params['step'] = '1';}
 else {$params['step'] = $compiler->_dequote($params['step']);}
 $name = $compiler->_dequote($params['name']);
 $props = array('index','index_prev','index_next','iteration','first','last','rownum','loop','show','total');
 if (in_array($name,$props)) {return $compiler->_syntax_error('Disallowed value (\''.$params['name'].'\') of parameter \'name\' in section tag.');}
 $props[] = $name;
