...
{
 if ($close) {return '';}
 if (!is_array($params)) {$params = $compiler->_parse_params($params);}
 $pre = '';
 if (isset($params['join']))
 {
  $fieldname = $compiler->_dequote($params['join']);
  if (!isset($compiler->block_props['form'])) {return $compiler->_syntax_error('Parameter \'join\' in tag element must be into a form.');}
  ...
  {
   if (!isset($params[$k])) {$params[$k] = 'Quicky_form::$forms[\''.$compiler->block_props['form'][0].'\']->'.$fieldname.'->'.$k;}
  }
  if (!isset($params['value'])) {$params['value'] = 'Quicky_form::$forms[\''.$compiler->block_props['form'][0].'\']->'.$fieldname.'->getValue()';}
  if (isset($params['value'])) {$params['value'] = 'Quicky_form::$forms[\''.$compiler->block_props['form'][0].'\']->'.$fieldname.'->getValue()';}
