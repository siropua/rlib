...
function quicky_block_form($params,$content,$compiler)
{
 ...
 var_dump($params);
 if (!isset($params['name']) && !isset($params['no_quicky_form'])) {return $compiler->_syntax_error('Missing parameter \'name\' in form tag.');}
 ...
 if ($mode)
 {
  if (!class_exists('Quicky_form'))
  {
   ...
   else {return $compiler->_syntax_error('Class Quicky_form isn\'t loaded');}
  }
  ...
  if (!isset(Quicky_form::$forms[$name]))
  {
   if ($auto_object)
   {
    ...
    foreach ($m as $v)
    {
     ...
     foreach ($params as $k => $v) {$params[$k] = $compiler->_dequote($v);}
     $code .= '$form->addElement('.var_export($params['name'],TRUE).','.var_export($params,TRUE).');'."\n";
    }
    $code .= '} ?>';
    eval('?>'.$code);
   }
   else {return $compiler->_syntax_error('Form \''.$name.'\' is not recognized');}
  }
 ...
 else {$content = $compiler->_tag_token($content,$block_name);}
 $s = '';
